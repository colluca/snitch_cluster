...
module snitch_fp_ss import snitch_pkg::*; #(
  ...
  // Mask for Selectively Enabling Scalar Chaining on Registers
    ...
    always_comb begin
      ...
      unique case (op_select[i])
        ...
        RegA, RegB, RegBRep, RegC, RegDest: begin
          ...
          op_ready[i] = (sc_active_q & sc_mask_q[fpr_raddr[i]]) ? sc_valid_q[fpr_raddr[i]] : (~sb_q[fpr_raddr[i]] & (~ssr_rvalid_o[i] | ssr_rready_i[i]));
          // Replicate if needed
      ...
      if (fpu_tag_out.ssr) begin
        ...
        if (sc_active_q & sc_mask_q[fpu_tag_out.rd]) begin
          if (sc_valid_q[fpu_tag_out.rd]) begin
            fpr_we = 1'b0;
            fpr_wready = 1'b0;
            if (((fpu_tag_out.rd == fpr_raddr[0]) | (fpu_tag_out.rd == fpr_raddr[1]) | (fpu_tag_out.rd == fpr_raddr[2])) & (fpu_in_valid | (lsu_qvalid & lsu_qready))) begin
              fpr_wready = 1'b1;
              fpr_we = 1'b1;
            end
          end
