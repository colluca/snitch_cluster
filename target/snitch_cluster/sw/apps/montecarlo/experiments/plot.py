    ...
    for impl in all_implementations:
        for n_cores in all_n_cores:
            ...
            ipc, throughput = calculate_ipc_throughput(results, impl, n_cores, n_samples,
                                                       batch_size)
            data.append({'impl': impl + str(n_cores), 'metric': 'ipc', 'value': ipc})
            data.append({'impl': impl + str(n_cores), 'metric': 'throughput', 'value': throughput})
    df = pd.DataFrame(data)
    df = df.pivot(index='metric', columns='impl', values='value')
    print(df)
