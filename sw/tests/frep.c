...
int main() { 
    ...
    asm volatile (
        ...
        : [ res0 ] "+f"(res_0), [ res1 ] "+f"(res_1), [ res2 ] "+f"(res_2), [ res3 ] "+f"(res_3), [ res4 ] "+f"(res_4), [ one ] "+f"(one)
        : [ n_iter_1 ] "r"(n_iter_1 - 1), [ n_iter_2 ] "r"(n_iter_2 - 1), [ n_iter_3 ] "r"(n_iter_3 - 1)
    );

    snrt_fpu_fence();
	return (exp_res_0 - res_0) + (exp_res_1 - res_1) + (exp_res_2 - res_2) + (exp_res_3 - res_3) + (exp_res_4 - res_4);
}
