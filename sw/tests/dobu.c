...
#define ALIGN_UP_TCDM(addr) ALIGN_NEXT_FROM_BASE(addr, SNRT_TCDM_START_ADDR, TCDM_ALIGNMENT)
...
int main() {
    ...
    double *bank24 = (double *)((uintptr_t)bank0 + NR_BANK_PER_HYPERBANK * BANK_SIZE);

    // Core 0 initializes bank 0 and 24 with data
    if (snrt_cluster_core_idx() == 0) {
        for (int i = 0; i < LEN; i++) {
            bank0[i * NR_BANK_PER_HYPERBANK] = 1;
            // *((uint64_t *)(bank0 + i * NR_BANK_PER_HYPERBANK)) = 0xAAAAAAAAAAAAAAAA;
        }
        for (int i = 0; i < LEN; i++) {
            bank24[i * NR_BANK_PER_HYPERBANK] = 2;
            // *((uint64_t *)(bank24 + i * NR_BANK_PER_HYPERBANK)) = 0xBBBBBBBBBBBBBBBB;
        }
    }
    ...
    if (snrt_cluster_core_idx() == 0) {
        ...
        for (int i = 0; i < LEN; i++) {
            ...
            // if (*((uint64_t *)(bank2 + i * NR_BANK_PER_HYPERBANK)) == 0xAAAAAAAAAAAAAAAA) {
                n_errors--;
            }
        }
        for (int i = 0; i < LEN; i++) {
            if (bank24[i * NR_BANK_PER_HYPERBANK] == 2) {
            // if (*((uint64_t *)(bank1 + i * NR_BANK_PER_HYPERBANK)) == 0xBBBBBBBBBBBBBBBB) {
                n_errors--;
            }
        }
    }
